# Assignment 1: Performance Analysis on a CPU #

**Due Friday Feb 20, 12:00noon**
...
## Environment Setup - Adroit and Slurm ##

__You will need to run code on Princeton's Adroit cluster for this assignment__
(Hostname would be `adroit.princeton.edu`). If for some reason you do not have a home directory on the machine, fill this form [here](https://forms.rc.princeton.edu/registration/?q=adroit).

For this assignment, we will use the Skylake compute nodes. These processors contain Intel Xeon Silver 4114 cores. Unlike consumer-grade processors with Hyper-Threading, these specific nodes are configured with one hardware thread per core.

These cores support AVX-512 vector instructions, which allow for simultaneous execution of the same SIMD operation on a vector of sixteen single-precision values (512 bits). For detailed architectural insights, you may refer to the Intel Skylake Mesh Architecture documentation.

Note: For grading purposes, we expect you to report on the performance of code run on the Skylake arch on Adroit, however
...
Adroit is a shared resource managed by the Slurm Workload Manager. To get accurate profiling results, you must not run your benchmarks on the login node. Instead, you must submit your jobs to compute nodes using sbatch or srun (we have provided the scripts to get you started).

Because multiple users share the cluster, profiling data can be affected by "resource contention" (interference from other users' tasks). To ensure your performance measurements are deterministic and reproducible:
...
1. gcc15 and ispc have already been added to adroit machines as modules, but they are not in the default module path. In order to use them:

```bash
# 1. Load additional module path. This line can be added to your .bashrc for permanence
export MODULEPATH=/scratch/network/zl1111/ece476-public/modulefiles:$MODULEPATH

# 2. Load the corresponding modules. You can add this to your .bashrc if you understand the implications.
...
make
```
For example, to build program 1,
```bash
cd prog1_mandelbrot_threads
make
```

Once you build the program, you can submit the SLURM jobs for the execution using the `run.sh` file provided. This script creates and launches the slurm script - `batch.sh`.

```bash
./run.sh <dir> <kernel> <cores> <kernel_opt>
...
view. To view the images, you can go to myadroit.princeton.edu -> Files, and navigate to the directory that you are working out of. 

As you can see in the images below, the result is a familiar and beautiful fractal. Each pixel in the image
corresponds to a value in the complex plane, and the brightness of each pixel is proportional to the computational cost of determining whether the value is contained in the Mandelbrot set. To get image 2, use the command option `--view 2`. (See function `mandelbrotSerial()` defined in `mandelbrotSerial.cpp`). You can learn more about the definition of the Mandelbrot set at <http://en.wikipedia.org/wiki/Mandelbrot_set>.
...
1. Modify the starter code (`workerThreadStart_v1()`) to parallelize the Mandelbrot generation using two processors. Specifically, compute the top half of the image in thread 0, and the bottom half of the image in thread 1. This type of problem decomposition is referred to as _spatial decomposition_ since different spatial regions of the image are computed by different processors. (Use `-i 0` for running the `workerThreadStart_v1()` implementation).
...
- *Hint:* Use `_prog2_cntbits` function helpful in this problem.
- Consider what might happen if the total number of loop iterations is not a multiple of SIMD vector width. We suggest you test your code with `./myexp -s 3`. *Hint:* You might find `_prog2_init_ones` helpful.
- *Hint:* Use `./myexp -l` to print a log of executed vector instruction at the end. Use function `addUserLog()` to add customized debug information in log. Feel free to add additional `prog2_Logger.printLog()` to help you debug.

The output of the program will tell you if your implementation generates correct output. If there are incorrect results, the program will print the first one it finds and print out a table of function inputs and outputs. Your function's output is after "output = ", which should match with the results after "gold = ". The program also prints out a list of statistics describing utilization of the fake vector units. You should consider the performance of your implementation to be the value "Total Vector Instructions". (You can assume every fake vector instruction takes one cycle on the fake SIMD CPU.) "Vector Utilization" shows the percentage of vector lanes that are enabled. 
...
2. Run `./myexp -s 10000` and sweep the vector width from 2, 4, 8, to 16. Record the resulting vector utilization. You can do this by changing the `#define VECTOR_WIDTH` value in `prog2_intrin.h`. Does the vector utilization increase, decrease or stay the same as `VECTOR_WIDTH` changes? Why?
...
1. Compile and run the program mandelbrot ispc. 
__The ISPC compiler is currently configured to emit 8-wide AVX2 vector instructions.__ 
...
If you look into detailed technical material about the CPUs in the adroit machines, you will find there are a complicated set of rules about how many scalar and vector instructions can be run per clock. For the purposes of this assignment, you can assume that there are about as many 8-wide vector execution units as there are scalar execution units for floating point math. 
...
2. There is a simple way to improve the performance of
 ...
 performance that exceeds the sequential version of the code by over 32 times (maybe way over)!
...
Note: Program 3 does not require you to write any additional code. The only
thing you need to do here is try it, write down your observations and analyze
...
4. _Extra Credit: (up to 2 points)_ Write your own version of the `sqrt` 
 function manually using AVX2 intrinsics. To get credit your implementation should be nearly as fast (or faster) than the binary produced using ISPC. You may find the [Intel Intrinsics Guide](https://software.intel.com/sites/landingpage/IntrinsicsGuide/) very helpful.
...
1. Prepare the data, then build the program and run. Use the command `ln -s /scratch/network/zl1111/ece476-public/pa1/data.dat ./data.dat`  to create a symbolic link to the dataset in your current directory (make sure you're in the `prog6_kmeans` directory). This is a large file (~800MB), so this is the preferred way to access it. However, if you'd like a local copy, you can run this command on your personal machine: `wget https://storage.googleapis.com/ece_476/data.dat`. Once you have the data, compile and run `kmeans` (it may take longer than usual for the program to load the data on your first try). The program will report the total runtime of the algorithm on the data.

2. Prepare the python environment for plotting.
```shell
# 1. Load the Anaconda module
module load anaconda3/2025.12

# 2. SETUP STORAGE FIRST (Critical Step) [Skip this if you have set up a similar thing before]
# We move/link the folder BEFORE creating the environment to avoid quota errors (your scratch space is much bigger than your home directory).
# Only run these two lines if you haven't already set up your scratch link.
# (If ~/.conda already exists, move it. If not, create the folder in scratch directly).
# $USER is your netid

if [ -d ~/.conda ]; then mv ~/.conda /scratch/network/$USER/; else mkdir -p /scratch/network/$USER/.conda; fi

# Create the symbolic link so ~/.conda points to your scratch space
ln -s /scratch/network/$USER/.conda ~/.conda

# 3. Create the environment
# Now that the link is there, this will actually write to /scratch/network...
conda create -n kmeans_p310 python=3.10

# 4. Activate and Install
conda activate kmeans_p310
pip install -r requirements.txt
```
Next, try running `python3 plot.py` which will generate the files "start.png" and "end.png" from the logs ("start.log" and "end.log") generated from running `kmeans`. These files will be in the current directory and should look similar to the above images. __Warning: You might notice that not all points are assigned to the "closest" centroid. This is okay.__ (For those that want to understand why: We project 100-dimensional datapoints down to 2-D using [PCA](https://en.wikipedia.org/wiki/Principal_component_analysis) to produce these visualizations. Therefore, while the 100-D datapoint is near the appropriate centroid in high dimensional space, the projects of the datapoint and the centroid may not be close to each other in 2-D.). As long as the clustering looks "reasonable" (use the images produced by the starter code in step 2 as a reference) and most points appear to be assigned to the clostest centroid, the code remains correct.
...
- __Important:__ you may only parallelize __one__ of the following functions: `dist`, `computeAssignments`, `computeCentroids`, `computeCost`. For an example of how to write parallel code using `std::thread`, see `prog1_mandelbrot_threads/mandelbrotThread.cpp`. Every time you modify the code, please remember to re-build it with the `make` command (as in [Getting Started](#getting-started)).
